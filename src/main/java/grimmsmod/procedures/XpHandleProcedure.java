...
public class XpHandleProcedure {
	public static void execute(Entity entity, double xpamount) {
		...
		if (ServerConfigConfiguration.INSTLEVELUP.get()) {
			while (entity.getData(GrimmsModVariables.PLAYER_VARIABLES).xp >= Math.round(Math.pow(entity.getData(GrimmsModVariables.PLAYER_VARIABLES).level * 10, 1.3))) {
				{
					GrimmsModVariables.PlayerVariables _vars = entity.getData(GrimmsModVariables.PLAYER_VARIABLES);
			...
			if (entity.getData(GrimmsModVariables.PLAYER_VARIABLES).xp >= Math.round(Math.pow(entity.getData(GrimmsModVariables.PLAYER_VARIABLES).level * 10, 1.3))) {
				{
					GrimmsModVariables.PlayerVariables _vars = entity.getData(GrimmsModVariables.PLAYER_VARIABLES);
					_vars.xp = entity.getData(GrimmsModVariables.PLAYER_VARIABLES).xp - Math.round(Math.pow(entity.getData(GrimmsModVariables.PLAYER_VARIABLES).level * 10, 1.3));
					_vars.syncPlayerVariables(entity);
